...
import static com.hxy.robot.api.constant.Constant.FILE_URL;
import static com.hxy.robot.api.constant.Constant.GROUP_BR;
import static com.hxy.robot.api.constant.Constant.GROUP_IDENTIFY;
import static com.hxy.robot.api.constant.Constant.INDEX_URL;
import static com.hxy.robot.api.constant.Constant.LOCATION_IDENTIFY;
import static com.hxy.robot.api.constant.Constant.WEB_PUSH_URL;
...
import com.hxy.robot.api.constant.Constant;
import com.hxy.robot.api.constant.StateCode;
import com.hxy.robot.api.enums.AccountType;
import com.hxy.robot.api.enums.ApiURL;
import com.hxy.robot.api.enums.RetCode;
import com.hxy.robot.api.model.Account;
import com.hxy.robot.api.model.DownLoad;
import com.hxy.robot.api.model.HotReload;
import com.hxy.robot.api.model.LoginSession;
import com.hxy.robot.api.model.Member;
import com.hxy.robot.api.model.Message;
import com.hxy.robot.api.model.Recommend;
import com.hxy.robot.api.model.SendMessage;
import com.hxy.robot.api.model.SyncCheckRet;
import com.hxy.robot.api.model.SyncKey;
import com.hxy.robot.api.model.WeChatMessage;
import com.hxy.robot.api.request.BaseRequest;
import com.hxy.robot.api.request.FileRequest;
import com.hxy.robot.api.request.JsonRequest;
import com.hxy.robot.api.request.StringRequest;
...
import com.hxy.robot.api.response.MediaResponse;
import com.hxy.robot.api.response.WebInitResponse;
import com.hxy.robot.api.response.WebSyncResponse;
...
public class WeChatApiImpl implements WeChatApi {
     ...
     */
    @Getter
    private Map<String,Map<String,Member>> memberGroupMap = new HashMap<>();
    ...
    public void login(boolean autoLogin) {
        ...
        if (autoLogin) {
            ...
            while (logging) {
                ...
                if (null == this.uuid) {
                    ...
                    ConfigRepository.put("finishInitWeChatFlag","true");
                }
        ...
        ConfigRepository.put("loginedFlag", "true");
        this.startRevive();
        this.logging = false;
        ...
        while (true) {
            ...
            for (Account account : memberList) {
                if (null != account.getUserName()) {
                    ...
                    log.info("------------->userName:",account.getUserName(),JSON.toJSONString(account));
                }
            }
    ...
    public void loadGroupList() {
        ...
        for (Account groupAccount : this.groupList) {
        	log.info("-------------》》》》》》groupAccount:{}",groupAccount);
        	if(accountMap.get(groupAccount.getUserName()) != null){
        		//把群组信息放到消息中去
        		accountMap.put(groupAccount.getUserName(), groupAccount);
        		log.info("映射组内成员信息》》》》》》》》》》》》》》》》》》》》");
        		Map<String,Member> memberMap = memberGroupMap.get(groupAccount.getUserName());
        		List<Member> memberList = groupAccount.getMembers();
        		if(memberList != null){
        			if(memberMap == null){
        				memberMap = new HashMap<>();
        				memberGroupMap.put(groupAccount.getUserName(), memberMap);
        			}
    ...
    private WeChatMessage processMsg(Message message) {
        ...
        if (message.isGroup()) {
            ...
            if (content.contains(GROUP_BR)) {
            	String memberId = content.substring(0, content.indexOf(GROUP_BR));
                content = content.substring(content.indexOf(GROUP_BR) + 6);
                //获取是哪个成员发送的信息
                Map<String,Member> memberMap = memberGroupMap.get(message.getFromUserName());
                if(memberMap == null){
                	this.loadGroupList();
                	memberMap = memberGroupMap.get(message.getFromUserName());
                }
        ...
        log.info("start-content:{}",content);
        ...
        if (null == fromAccount) {
        	...
        	if(MapperRepository.get(message.getFromUserName()) == null){
                ...
                if(robotServices != null){
            		for(TRobotServiceDao robot : robotServices){
            			log.info("groupNickName:"+nickName+ ", robotService描述:" + robot.getServiceDesc() + ", robotServiceType:"+ robot.getServiceType());
                		if((StringUtils.isNotEmpty(robot.getServiceDesc()) && robot.getServiceDesc().contains(nickName)) || (StringUtils.isNotEmpty(nickName) && nickName.contains(robot.getServiceDesc()))){
                ...
                log.info("初始化MapperRepository:{}",MapperRepository.map);
        	}
            ...
            if (log.isDebugEnabled()) {
                log.info("------收到消息JSON: {}", WeChatUtils.toJson(message));
            }
            log.info("------收到消息JSON: {}", WeChatUtils.toJson(message));
        }
    ...
    public MediaResponse uploadMedia(String toUser, String filePath) {
        ...
        String mediaId = System.currentTimeMillis() / 1000 + com.hxy.robot.utils.StringUtils.random(6);
    ...
    public boolean sendImg(String toUserName, String filePath) {
        ...
        String msgId = System.currentTimeMillis() / 1000 + com.hxy.robot.utils.StringUtils.random(6);
